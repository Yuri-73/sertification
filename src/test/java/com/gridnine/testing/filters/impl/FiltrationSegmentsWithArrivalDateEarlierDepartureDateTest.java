...
class FiltrationSegmentsWithArrivalDateEarlierDepartureDateTest {
    ...
    void filter() {
        ...
        Flight flight2 = new Flight(segments2); //для фильтра-1: рейс вылета ранее текущей даты

        List<Segment> segments3 = new ArrayList<>();
        segments3.add(segment3);
        Flight flight3 = new Flight(segments3); //для фильтра-2:рейс времени вылета позже времени прибытия

        List<Segment> segments4 = new ArrayList<>();
        segments4.add(segment4);
        segments4.add(segment5);
        Flight flight4 = new Flight(segments4); //для фильтра-3: рейс с наземным временем простоя более двух часов
        ...
        //check:
        assertEquals(segmentsWithArrivalDateEarlierDepartureDate.filter(flights), flights2);
        assertEquals(flights2.size(), 3);  //3 фильтрации
    }
}